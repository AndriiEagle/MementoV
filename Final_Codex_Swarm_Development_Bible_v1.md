...
### 1.5. `OSS технічний due diligence для voice-first memoir продукту.docx`

Ключевые выводы:

- самый чистый OSS-first путь для этого продукта не в том, чтобы клонировать случайные voice-agent репозитории, а в том, чтобы собрать очень конкретный гибрид:
  - `livekit/agents-js` как realtime core;
  - `livekit-examples/agent-starter-react` как UI/bootstrap fork;
  - `openai/openai-agents-js` только для server-side deep biographer layer;
  - `mem0ai/mem0` только как retrieval-memory слой, а не как canonical truth;
  - `langfuse/langfuse` как self-hosted observability/evals spine;
- `Postgres` остаётся канонической базой истины: interview state, people, places, events, fact claims, unresolved topics, chapter jobs и chapter versions не должны жить внутри memory SDK;
- `Pipecat` силён как reference и backup strategy, но для `Next.js + TypeScript + Railway + Postgres` на старте добавляет лишнюю runtime и cognitive complexity;
- `LangGraphJS` разумен только позже для длинных async chapter workflows с interrupts и manual review, а не на live hot path;
- `openai/openai-realtime-agents`, `pipecat-flows`, `Mastra` и архивные quickstart-репозитории не должны быть foundation для MVP;
- самый важный инфраструктурный запрет: self-hosted `LiveKit` media plane нельзя сажать на `Railway`; Railway остаётся местом для web/API/workers, но не для production WebRTC media plane.
...
### 3.1. OSS foundation rules

- Форкать стоит только тонкие оболочки и стартовые UI-базы, если это реально экономит недели.
- Core OSS движки лучше брать как dependency и обновляемый upstream, а не как форк.
- Для этого продукта допустимый MVP-fork only one:
  - `livekit-examples/agent-starter-react`
- Для этого продукта брать как packages/upstream:
  - `livekit/agents-js`
  - `openai/openai-agents-js`
  - `mem0ai/mem0`
  - `langfuse/langfuse`
- Для этого продукта держать только как reference:
  - `pipecat-ai/pipecat`
  - `langchain-ai/langgraphjs`
  - `getzep/graphiti`
- Для этого продукта не брать как MVP foundation:
  - `openai/openai-realtime-agents`
  - `pipecat-ai/pipecat-flows`
  - `Mastra`
  - архивные Pipecat quickstart repos
...
| Voice UI bootstrap | fork `livekit-examples/agent-starter-react`, then simplify aggressively |
...
| Realtime orchestration | `livekit/agents-js` |
| Deep biographer orchestration | `openai/openai-agents-js`, server-side only |
| DB | PostgreSQL |
| ORM | Prisma или Drizzle, но выбрать один и не мешать |
| Vector memory | pgvector |
| Retrieval memory | `mem0ai/mem0`, secondary only |
| Storage | S3-compatible or R2 |
| STT | Whisper / speech-to-text |
| LLM | GPT-class model for dialogue + chapter synthesis |
| Observability / evals | self-hosted `langfuse/langfuse` |
| Web/API/workers deploy | Railway |
| Media plane | separate `LiveKit` deployment, not Railway |
| Analytics | Plausible or PostHog |
| Email | Resend or Postmark |
| Payments | Stripe, not in first shipping wave |

### 7.1. Clone / Fork / Reference policy

| Категория | Что делать |
|---|---|
| `Fork this` | `livekit-examples/agent-starter-react` |
| `Use as foundation dependency` | `livekit/agents-js`, `openai/openai-agents-js`, `mem0ai/mem0`, `langfuse/langfuse` |
| `Reference only` | `pipecat-ai/pipecat`, `langchain-ai/langgraphjs`, `getzep/graphiti` |
| `Do not use as MVP foundation` | `openai/openai-realtime-agents`, `pipecat-ai/pipecat-flows`, `Mastra`, archived quickstarts |

### 7.2. What this changes in the 360-step plan

Высокоуровневый порядок шагов не меняется.

Меняется substrate:

- realtime front door теперь должен проектироваться вокруг `livekit/agents-js`, а не вокруг абстрактного voice layer;
- ECHO voice UI можно и нужно стартовать не с нуля, а через сильно упрощённый fork `agent-starter-react`;
- deep biographer нужно держать server-side через `openai-agents-js`, а не вплетать в media plane;
- memory SDK теперь явно вторичен по отношению к `Postgres`;
- observability/evals теперь нужно считать обязательным инженерным слоем, а не optional polish;
- deployment split зафиксирован: `Railway` для app/workers, отдельный media plane для `LiveKit`.
